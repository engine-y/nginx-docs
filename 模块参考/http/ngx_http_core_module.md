...
如果禁用，nginx 发出的重定地址向将是相对路径，否则为绝对路径。
...
在 FreeBSD 和 Linux 上启用或禁用异步文件 I/O（AIO）模式：
...
```
在 Linux 上，[directio](#directio) 只能用于读取 512 字节边界对齐的块（或 XFS 为 4K）。文件未对齐的末端以阻塞模式读取。对于字节范围请求和不是从文件开头开始的 FLV 请求也是如此：在文件的开头和结尾读取未对齐的数据将被阻塞。

当在 Linux 上同时启用 AIO 和 [sendfile](#sendfile) 时，AIO 用于大于或等于 [directio](#directio) 指令指定大小的文件，而 [sendfile](#sendfile) 用于较小的文件（禁用 [directio](#directio) 时也是如此）。
...
读取和发送文件操作被卸载到指定[线程池](#thread_pool)中的线程。如果省略线程池名称，则使用名称为 `default` 的线程池。线程池名也可以用变量设置：

```nginx
aio threads=pool$disk;
```

默认情况下，多线程模式是不启用的，需要自行使用 `--with-threads` 配置参数编译启用。目前，多线程仅与 [epoll](http://nginx.org/en/docs/events.html#epoll)、[kqueue](http://nginx.org/en/docs/events.html#kqueue) 和 [eventport](http://nginx.org/en/docs/events.html#eventport) 方式兼容。仅在 Linux 上支持多线程发送文件。
...
如果启用了 [aio](#aio)，则指定它是否用于写入文件。目前，这仅在使用 aio 线程时有效，并且仅限于将从代理服务器接收的数据写入临时文件。
...
如果在使用正则表达式定义的 location 内使用了别名，那么这种正则表达式应该包含捕获（即正则匹配到的内容），并且别名应该引用这些捕获（0.7.40），例如：
...
|**默认**|auth_delay 0s;|
|**上下文**|http、server、location|
|**提示**|该指令在 1.17.10 版本中出现|

当访问受到 [密码](ngx_http_auth_basic_module.md)、[子请求结果](ngx_http_auth_request_module.md)或[JWT](ngx_http_auth_jwt_module.md)限制时，使用 401 响应代码延迟处理未经授权的请求，以防止定时攻击。
...
允许在 HTTP/1.1 中禁用分块传输编码。尽管标准有要求，但在使用不支持分块编码的软件时，它可能会派上用场。
...
确定 nginx 是否应将整个客户端请求体保存在单个缓冲区中。在使用 `$request_body` 变量时，建议使用该指令，用来节省所涉及的复制操作的数量。
...
定义一个目录，用于存储客户端请求正文的临时文件。指定目录下最多可以使用三级子目录层次结构。例如以下配置
...
定义读取客户端请求头的超时时间。如果客户端在这段时间内没有传输整个请求头，则将 408（请求超时）错误返回给客户端。
...
允许精确调整每个连接的内存分配。该指令对性能影响很小，一般**不建议**使用。默认情况下，32 位平台上的大小为 256 字节，64 位平台上为 512 字节。
...
当读取大于或等于指定大小的文件时，开启使用 `O_DIRECT` 标志（FreeBSD、Linux）、`F_NOCACHE` 标志（macOS）或 `directio()` 函数（Solaris）。该指令自动禁用（0.7.15）给定请求使用 [sendfile](#sendfile)。它对于提供大文件很有用：

```nginx
directio 4m;
```

或者在 Linux 上使用 [aio](#aio) 时。
...
- `if_not_owner`
  ...
  如果路径名的任何组成部分是符号链接，并且链接指向的链接和对象为不同所有者，则拒绝访问文件。
...
此指令仅适用于具有 `openat()` 和 `fstatat()` 接口的系统。这样的系统包括近期版本的FreeBSD、Linux 和 Solaris。
...
限制通过一个保持活动连接可以处理请求的最长时间。达到这个时间后，连接在后续请求处理后关闭。
...
限制 location 内允许的 HTTP 方法。`method` 可以是以下之一：`GET`、`HEAD`、`POST`、`PUT`、`DELETE`、`MKCOL`、`COPY`、`MOVE`、`OPTIONS`、`PROPFIND`、`PROPPATCH`、`LOCK`、`UNLOCK` 或 `PATCH`。允许 `GET` 方法也将使得 `HEAD`方法被允许。可以使用 [ngx_http_access_module](ngx_http_access_module.md) 和 [ngx_http_auth_basic_module](ngx_http_auth_basic_module.md) 模块指令来限制对其他方法的访问：
...
参数值可以包含变量（1.17.0），在需要根据特定条件限制速率的情况下，它可能很有用：
```nginx
map $slow $rate {
    1     4k;
    2     8k;
...
也可以在 `$limit_rate` 变量中设置速率限制，但是，在1.17.0以后的版本中不再推荐使用：
...
设置初始量，在此之后向客户端的响应的进一步传输将受到速率限制。参数值可以包含变量（1.17.0）。
...
默认值“on”指示 nginx 在完全关闭连接之前[等待](#lingering_timeout)并[处理](#lingering_time)来自客户端的额外数据，但前提是预判客户端可能正在发送更多数据。

值“always”将导致 nginx 无条件地等待和处理额外的客户端数据。

值“off”告诉 nginx 永远不要等待更多数据并立即关闭连接。这种行为违反了协议，在正常情况下不应使用。

要控制关闭 HTTP/2连接，必须在服务器级别 (1.19.1) 指定该指令。
...
在重定向中主服务器名称的使用由 [server_name_in_redirect](#server_name_in_redirect) 指令控制。
...
允许精确调整每个请求的内存分配。该指令对性能影响很小，一般不建议使用。
...
可选参数status_zone (1.17.1) 允许收集指定的请求和响应的 DNS 服务器统计信息zone。该参数作为我们 商业订阅的一部分提供。
...
```
该/data/w3/i/top.gif文件将被发送以响应“ /i/top.gif”请求。

该path值可以包含变量，除了$document_root和$realpath_root。

仅通过将 URI 添加到root指令的值来构造文件的路径。如果必须修改 URI， 则应使用 [alias](#alias)指令。
...
> 多种认证鉴权指令并存时的决策策略，任意一个通过就行，还是全部通过才行
...
设置存储子请求响应正文的缓冲区`size`。默认情况下，缓冲区大小等于一内存页。可以是 4K 或 8K，具体取决于平台。然而，它可以设置的更小。

该指令仅适用于将响应主体保存到内存中的子请求。例如，此类子请求由 SSI创建。
...
将文件扩展名映射到 MIME 类型的响应。扩展名不区分大小写。可以将多个扩展映射到一种类型，例如：
...
一个足够完整的映射表与 conf/mime.types文件中的 nginx 一起分发。

要使特定的location下的所有请求的MIME类型都用"application/octet-stream为所有请求发出“，可以使用以下配置：
